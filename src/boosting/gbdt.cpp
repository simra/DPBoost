...
data_size_t GBDT::BaggingHelper(Random& cur_rand, data_size_t start, data_size_t cnt, data_size_t* buffer) {
  ...
  if(config_->boost_method == std::string("infocom")){
    //should also prune the instances with gradient bigger than x.
...
//    CHECK(cur_left_cnt == bag_data_cnt);
    ...
    if(config_->boost_method == std::string("dpboost_2level")) {
      ...
      if(config_->balance_partition == 1){
        if(config_->num_iterations % config_->inner_boost_round != 0){
          if((iter_  / config_->inner_boost_round) == (config_->num_iterations / config_->inner_boost_round)){
            bag_data_cnt = static_cast<data_size_t>(cnt/(config_->num_iterations % config_->inner_boost_round));
          }
          else{
            bag_data_cnt = static_cast<data_size_t>(cnt/config_->inner_boost_round);
          }
        }
        else
          bag_data_cnt = static_cast<data_size_t >(cnt/config_->inner_boost_round);
        ...
        bag_data_cnt = static_cast<data_size_t>(std::pow(base, iter_inside) * (1 - base) /
                                                (1 - std::pow(base, config_->inner_boost_round)) * cnt);
      }
    }
    else {
      iter_inside = iter_;
      bag_data_cnt= static_cast<data_size_t>(std::pow(base, iter_inside) * (1 - base) /
                                 (1 - std::pow(base, config_->num_iterations)) * cnt);
    }
    data_size_t cur_left_cnt = 0;
    data_size_t cur_right_cnt = 0;
    data_size_t chosen_data_left_cnt = 0;
      ...
      if(!already_chosen[i]){
        ...
        if(cur_rand.NextFloat() < prob){
          ...
          if(grad - threshold <= 1e-6){
            ...
            filter_data_cnt++;
            bag_left_cnt++;
            *(right_buffer - bag_left_cnt) = start + i;
          }
          cur_pass_cnt++;
        }
        else{
...
//        iter_inside = iter_ % (config_->num_iterations / config_->high_level_boost_round);
      ...
      total_budget = total_budget / num_tree_per_iteration_;
//      std::cout<<"total_budget:"<<total_budget<<std::endl;
      if(total_budget != 0) {
        ...
        double base = 1 - config_ -> learning_rate;
        new_tree->proportional_prune(iter_inside, base, config_->boost_method, g_m, config_->geo_clip);
        float lamda = 0.1;
        ...
        if(config_->boost_method == std::string("dpboost_2level")){
          ...
          sum = (change_round * std::pow(g_m / (1 + lamda), 2.0/3.0) +
                 std::pow(2*std::pow(base,change_round) * g_m ,2.0/3.0) * (1-std::pow(base,2.0 / 3.0 *(total_iter-change_round))) / (1-std::pow(base, 2.0/3.0)));
        }
        double sensitivity;
        if (iter_inside + 1 <= change_round) {
          sensitivity = g_m / (1 + lamda);
        } else {
          sensitivity = 2 * std::pow(base, iter_inside) * g_m;
        }
        ...
        if(config_->boost_method == std::string("dpboost")) {
          ...
          if(config_->num_iterations % config_->inner_boost_round == 0) {
            ...
            current_budget = total_budget / (config_->num_iterations / config_->inner_boost_round + 1);
          }
          laplace_scale = sensitivity / current_budget;
        }
        else{
...
//        std::cout << "global_total_budget:" << global_total_budget << std::endl;
  ...
  if(config_->boost_method == "infocom") {
    if (!should_continue) {
